...
/**
 * Shared game states for Safari Zone minigames.
 */
enum class SafariGameState {
    START,
    PLAYING,
    RESULT
}
...
/**
 * Centralized data for the Safari Zone.
 */
object PokemonConstants {
    val KANTO_POKEMON_LIST = listOf(
    ...
    /**
     * Helper to get a clean name for Word Search (all caps, no special chars)
     */
    fun getCleanPokemonName(index: Int): String {
        return KANTO_POKEMON_LIST[index]
            .uppercase()
            .replace("♂", "")
            .replace("♀", "")
            .replace("'", "")
